...
Public Class P_articulo_ED
    ...
    Private Sub txtid_articulo_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtid_articulo.KeyPress
        If e.KeyChar = ChrW(13) And Len(txtid_articulo.Text) > 4 Then
            If Not Articulo.Existe(txtid_articulo.Text) Then
                M("¡El articulo " + txtid_articulo.Text + " no existe!", 3)
                txtid_articulo.Text = ""
                txtid_articulo.Focus()
            Else
                Campos(True)

                Tabla = Articulo.Consulta(txtid_articulo.Text)
